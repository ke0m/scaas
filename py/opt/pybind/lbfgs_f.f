...
C
      ...
      REAL X(N),G(N),DIAG(N),W(N*(2*M+1)+2*M)
      REAL F,EPS,XTOL
      LOGICAL DIAGCO
C
...
C 
C 
C     DIAG    is a REAL array of length N. If DIAGCO=.TRUE.,
...
C 
C 
C     EPS     is a positive REAL variable that must be set by
...
C 
C     XTOL    is a  positive REAL variable that must be set by
...
C             is less than XTOL.
C 
C     W       is a REAL array of length N(2M+1)+2M used as
...
C 
C    GTOL is a REAL variable with default value 0.9, which
...
C 
C    STPMIN and STPMAX are non-negative REAL variables which
...
C 
C    Other routines called directly:  SAXPY, SDOT, LB1, MCSRCH
C 
...
C     - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
C
      REAL GTOL,ONE,ZERO,GNORM,SDOT,STP,STP1,FTOL,STPMIN,
     .                 STPMAX,YS,YY,SQ,YR,BETA,XNORM
...
C
      ...
      REAL X(N),G(N),F,GNORM,STP,GTOL,STPMIN,STPMAX
      LOGICAL FINISH
      COMMON /LB3/MP,LP,GTOL,STPMIN,STPMAX
C
      IF (ITER.EQ.0)THEN
...
C
      ...
      COMMON /LB3/MP,LP,GTOL,STPMIN,STPMAX
      DATA MP,LP,GTOL,STPMIN,STPMAX/6,6,9.D-01,1.0D-10,1.0D10/
      END
C
C
C   ----------------------------------------------------------
