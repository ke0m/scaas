...
#shift = np.random.randint(-1,2,nrx)
...
for isx in range(nsx):
  ...
  allrecz[isx,:] = np.zeros(len(recs),dtype='int32') + reczp #+ shift
...
if(plotacq):
  ...
  plt.grid()
  if(verb):
    print("Final receiver position: %d"%(allrecx[0,-1]))
    print("Final source position: %d"%(allsrcx[-1,0]))
  plt.show()
