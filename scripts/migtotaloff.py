"""
Migrated extended image for multiple shots
...
plt.figure(1)
# Plot velocity model
vmin = np.min(dvelp); vmax = np.max(dvelp)
plt.imshow(dvelp,extent=[0,nxp,nzp,0],vmin=vmin,vmax=vmax,cmap='gray')
...
iaxes = seppy.axes([nz,nx,rnh],[0.0,0,-zidx*dx],[dz,dx,dx])
imgpt = np.transpose(imgp,(1,2,0))
imgptwind = imgpt[bz+5:bz+5+nz,bx+5:bx+5+nx]
greyargs = "gainpanel=a wantscalebar=y pclip=100"
sep.pltgrey(iaxes,imgptwind,greyargs)

sep.write_file(None,iaxes,imgptwind,ofname='extpt.H')
