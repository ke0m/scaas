"""
Imaging/modeling data based on a streamer
acqusition geometry
...
class streamergeom:
  ...
  field data streamer geometry
  """
  def __init__(self,nx,dx,ny,dy,nz,dz,nrec,srcxs=None,srcys=None,recxs=None,recys=None,
               ox=0.0,oy=0.0,oz=0.0,maxx=None,maxy=None):
    """
    Creates a streamer geometry object for split-step fourier downward continuation.
    ...
    Parameters:
      ...
      ox    - x-axis origin [0.0]
      oy    - y-axis origin [0.0]
      oz    - z-axis origin [0.0]
      maxx  - maximum x-distance to use [maximum iline offset for the shot]
      maxy  - maximum y-distance to use [maximum xline offset for the shot]

    Returns:
      a streamer geom object
    """
    # Spatial axes
    ...
    # Angle
    self.__na,self.__oa,self.__da = None,None,None

    # Get minimum and maximum offsets
    maxoffxs = np.zeros(self.__nexp,dtype='float32')
    maxoffys = np.zeros(self.__nexp,dtype='float32')
    ntr = 0
    for iexp in range(self.__nexp):
      # Compute the offsets for this shot
      sy,sx = self.__srcys[iexp],self.__srcxs[iexp]
      offx = self.__srcxs[iexp] - self.__recxs[ntr:ntr+self.__nrec[iexp]]
      offy = self.__srcys[iexp] - self.__recys[ntr:ntr+self.__nrec[iexp]]
      maxoffx = np.max(np.abs(offx))
      maxoffy = np.max(np.abs(offy))
      ntr += self.__nrec[iexp]

    self.__maxx = np.max(maxoffx)
    self.__maxy = np.max(maxoffy)
    if(maxx is not None):
      print(maxx, self.__maxx)
      if(maxx < self.__maxx):
        print("Warning, provided maxx is less than maximum offset. Setting to maximum inline offset")
      else:
        self.__maxx = maxx
    if(maxy is not None):
      if(maxy < self.__maxy):
        print("Warning, provided maxy is less than maximum offset. Setting to maximum xline offset")
      else:
        self.__maxy = maxy

    # Windowed sizes for image and velocity model
    self.__nxw = int((self.__maxx)/self.__dx + 0.5)
    self.__nyw = int((self.__maxy)/self.__dy + 0.5)
    if(self.__nyw == 0): self.__nyw += 1

    # TODO: take min and max offset, how much of the model does this cover?
    # Thresholds for handling boundaries
    self.__wdist = 0.25*self.__maxx
    self.__edist = 1 - self.__wdist
    self.__sdist = 0.5*self.__maxy
    self.__ndist = self.__sdist
  ...
  def image_data(self,dat,dt,minf,maxf,vel,jf=1,nhx=0,nhy=0,sym=True,nrmax=3,eps=0.,dtmax=5e-05,wav=None,
    ...
    datw = np.zeros([self.__nyw,self.__nxw,self.__nwc],dtype='complex64')
    # Allocate the source for one shot
    sou = np.zeros([self.__nwc,self.__nyw,self.__nxw],dtype='complex64')

    # Single square root object
    ssf = ssr3(self.__nxw ,self.__nyw,self.__nz ,   # Spatial Sizes
               self.__dx  ,self.__dy ,self.__dz ,   # Spatial Samplings
               self.__nwc,self.__ow,self.__dwc,eps, # Frequency axis
               ntx,nty,px,py,                       # Taper and padding
               dtmax,nrmax,nthrds)                  # Reference velocities and threads
    ...
    if(nhx == 0 and nhy == 0):
      imgtmp = np.zeros([self.__nz,self.__nyw,self.__nxw],dtype='float32')
      ...
      if(sym):
        ...
        imgtmp = np.zeros([self.__rnhy,self.__rnhx,self.__nz,self.__nyw,self.__nxw],dtype='float32')
    ...
    sep = seppy.sep()
    ntr,dsx = 0,0
    begx,endx = self.__ox,self.__ox + self.__maxx
    begy,endy = self.__oy,self.__oy + self.__maxy
    bx,ex = 0,self.__nxw
    by,ey = 0,self.__nyw
    for iexp in progressbar(range(self.__nexp),"nexp:",verb=sverb):
      # Get the source coordinates
      sy = self.__srcys[iexp]; sx = self.__srcxs[iexp]
      isy = int((sy-begy)/self.__dy+0.5); isx = int((sx-begx)/self.__dx+0.5)
      # Create the source wavefield for this shot
      sou[:] = 0.0
      sou[:,isy,isx]  = wfftd[:]
      #plot_img2d(np.real(sou[:,0,:]))
      #TODO: Wrap this in a helper function
      # Window the velocity model for this shot
      if(ntr != 0):
        # Window in x
        dsx = sx - self.__srcxs[iexp-1]
        if(sx - self.__ox > self.__wdist):
          if(endx + dsx >= self.__ox + (self.__nx)*self.__dx):
            offsx = self.__ox + (self.__nx)*self.__dx - endx
            endx  = self.__ox + (self.__nx)*self.__dx
            begx += offsx
          else:
            begx += dsx; endx += dsx
        bx = int((begx - self.__ox)/self.__dx + 0.5)
        ex = int((endx - self.__ox)/self.__dx + 0.5)
        if(ex == 0): ex += 1
        # Window in y
        dsy = sy - self.__srcys[iexp-1]
        if(sy - self.__oy > self.__sdist):
          if(endy + dsy > self.__oy + (self.__ny-1)*self.__dy):
            offsy = self.__oy + (self.__ny-1)*self.__dy - end
            endy  = self.__oy + (self.__ny-1)*self.__dy
            endy += offsy
          else:
            begy += dsy; endy += dsy
        by = int((begy - self.__oy)/self.__dy + 0.5)
        ey = int((endy - self.__oy)/self.__dy + 0.5)
        if(ey == 0): ey += 1
      slow = np.ascontiguousarray(slo[:,by:ey,bx:ex])
      #plot_vel2d(slow[:,0,:])
      pltrecxs = self.__recxs[ntr:ntr+self.__nrec[iexp]]
      zeros = np.zeros(len(pltrecxs),dtype='float32') + self.__dz*10
      #fig = plt.figure(); ax = fig.gca()
      #ax.imshow(1/slow[:,0,:],cmap='jet',extent=[begx,endx,(self.__nz-1)*self.__dz,0])
      #ax.scatter(sx,self.__dz*10,c='tab:red',marker='*')
      #ax.scatter(pltrecxs,zeros,c='tab:green',marker='v')
      #plt.show()
      # Compute slowness and reference slownesses
      ssf.set_slows(slow)
      # Inject the data for this shot
      datw[:] = 0.0
      ssf.inject_data(self.__nrec[iexp],self.__recys[ntr:],self.__recxs[ntr:],begy,begx,dfftd[ntr:,:],datw)
      datwt = np.ascontiguousarray(np.transpose(datw,(2,0,1))) # [ny,nx,nwc] -> [nwc,ny,nx]
      #plot_img2d(np.real(datwt[:,0,:]))
      # Initialize temporary image
      imgtmp[:] = 0.0
      if(nhx == 0 and nhy == 0):
        # Conventional imaging
        ssf.migallw(datwt,sou,imgtmp,wverb)
        #plot_img2d(imgtmp[:,0,:],pclip=0.2)
      else:
        # Extended imaging
        ssf.migoffallw(datwt,sou,imgtmp,wverb)
      #  TODO: apply a taper before padding
      # Pad in x
      bpadx = bx
      epadx = self.__nx - ex
      # Pad in y
      bpady = by
      epady = self.__ny - ey
      imgtmppad = np.pad(imgtmp,((0,0),(bpady,epady),(bpadx,epadx)))
      tag = create_inttag(iexp,999)
      sep.write_file("./dat/streamer/imgstm-%s.H"%(tag),imgtmppad[:,0,:])
      #plot_img2d(imgtmppad[:,0,:],pclip=0.2)
      oimg += imgtmppad
      # Increase number of traces
      ntr += self.__nrec[iexp]
