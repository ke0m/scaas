...
def create_ptscatmodel(nz, nx, j1, j2, verb=False):
  ...
  for ib1 in range(nb[0]):
    for ib2 in range(nb[1]):
      ...
      scats[ct1, ct2] = 1.0
      if (verb):
        print("Block %d %d: (%d,%d)" % (ib1, ib2, ct1, ct2))
...
def create_randomptb(
  ...
  """
  noise = noise_generator.perlin(
      x=np.linspace(0, nptsx, nx),
      y=np.linspace(0, nptsz, nz),
      octaves=octaves,
      period=period,
      Ngrad=Ngrad,
      persist=persist,
...
def create_randomptb_loc(
  ...
  """
  noise = noise_generator.perlin(
      x=np.linspace(0, nptsx, nax),
      y=np.linspace(0, nptsz, naz),
      octaves=octaves,
      period=period,
      Ngrad=Ngrad,
      persist=persist,
  ...
  if (px1 < 0):
    cx = int(nax / 2)
  px2 = nx - cx - int(nax / 2)
  noisep = np.pad(noiseout, ((pz1, pz2), (px1, px2)),
                  'constant',
                  constant_values=1)
...
def create_randomptbs_loc(
  ...
  def randfloat(minf, maxf):
    if maxf > minf:
      raise ValueError("maxf must be greater than minf")
...
def create_layered(
  ...
  for iref in range(nref):
    ...
    # Calculate shifts
    if (flat is False):
      shp = noise_generator.perlin(
          x=np.linspace(0, npts, nx),
          octaves=octaves,
          period=80,
          Ngrad=80,
          persist=persist,
    ...
    if (interp == 'lint'):
      lyrs += np.array([
          flt.interpolation.shift(rpt[:, ix], shp[ix], order=1)
          for ix in range(nx)
      ]).T
    else:
...
def insert_circle(vel, dx, dz, centerx, centerz, rad, val) -> np.ndarray:
  ...
  zt = centerz - rad
  zb = centerz + rad
  lxbnd = np.zeros(len(zvals))
  rxbnd = np.zeros(len(zvals))
